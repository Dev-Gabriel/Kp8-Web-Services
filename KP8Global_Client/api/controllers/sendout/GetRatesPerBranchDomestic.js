...
var getRatesPerBranchDom  = function (req, mainCB) {
    ...
    console.log('-------------- in Get Rates Per Branch Domestic ------------------')
    async.waterfall([
        // function(cb){
        ...
        function(cb){
            ...
            dbTrans1.connection.query("SELECT nextID,currID,nDateEffectivity,cDateEffectivity,cEffective,nextID, NOW() as currentDate FROM kpformsglobal.ratesperbranchheaderG WHERE cEffective = 1 and branchcode = ? and zonecode = ?;"    
        ,[bcode, zcode]
        ,function(error, result){
            ...
            console.log('currentDate : ' + currentDate)
                if (nextID == 0 || (new Date(nDateEffectivity) - new Date(currentDate)) > 0)
                {
                    ...
                    dbTrans2.connection.query("SELECT ChargeValue AS charge FROM kpformsglobal.ratesperbranchchargesG WHERE ROUND(?,2) BETWEEN MinAmount AND MaxAmount AND `type` = ?;"
                    ,[amount, currID]
                    ,function(error, result){
                        ...
                        dbTrans3.connection.beginTransaction(function (errorTransaction){
                            ...
                            else 
                            {
                                async.parallel([function(parCB)
                                    {
                                    // 1st update
                                        dbTrans3.connection.query("update kpformsglobal.ratesperbranchheaderG SET  cEffective = 2 where cEffective = 1 and branchcode = ? and zonecode = ?;"
                                        ,[bcode, zcode]
                                        ,function(error, result){
                                            parCB(error)
                                        })
                                    ...
                                    {
                                    // 2nd update
                                        dbTrans3.connection.query("update kpformsglobal.ratesperbranchheaderG SET cEffective = 1 where currID = ? and branchcode = ? and zonecode = ?;"
                                        ,[nextID,bcode, zcode]
                                        ,function(error, result){
                                            parCB(error)
                                        })
                                    ...
                                    if(error){
                                        ...
                                        logs.info("SUCCESS:: UPDATE kpformsglobal.ratesperbranchheaderG: SET cEffective: 2 WHERE cEffective: 1 AND branchcode: " + bcode + " AND zonecode: " + zcode);
                                        logs.info("SUCCESS:: UPDATE kpformsglobal.ratesperbranchheaderG: SET cEffective: 1 WHERE currID: " + nextID + " AND branchcode: " + bcode + " AND zonecode: " + zcode);
                                        ...
                                        dbTrans4.connection.query("SELECT ChargeValue AS charge FROM kpformsglobal.ratesperbranchchargesG WHERE ROUND(?,2) BETWEEN MinAmount AND MaxAmount AND `type` = ?;"
                                        ,[amount, currID]
                                        ,function(error, result){
...
module.exports = getRatesPerBranchDom;